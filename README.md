...
- Provision and maintain RDS PostgreSQL database for persistent data storage
- Deploy and manage MSK Kafka cluster for real-time event-driven data streaming across microservices
- Configure Application Load Balancer (ALB) for frontend services
- Create and maintain entire infrastructure using Terraform (IaC) with remote state management
- Build and manage Jenkins CI/CD pipelines for automated builds, tests, and deployments to ECS Fargate
...
- Deployed and managed AKS clusters with Helm charts for consistent releases. Implemented GitOps workflows using ArgoCD for continuous sync between Git and clusters
...
- Configured AWS Cloud Map with private namespaces enabling secure backend service-to-service communication without exposing to public internet
- Provisioned RDS PostgreSQL database for data storage with automated backups
...
- Provisioned RDS PostgreSQL database for persistent data storage with automated backups
...
- Deployed workloads run Azure Kubernetes Service (AKS) using Helm charts to manage Kubernetes resources and ensure consistent application releases.
...
- The second pipeline also updated the image tag in Kubernetes files through a script, which was then deployed automatically to the cluster using ArgoCD.
